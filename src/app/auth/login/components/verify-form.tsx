...
export default function VerifyForm({ onSubmit, setShowMfa, loading = false, email = "example@mail.com" }: VerifyForm) {
	...
	return (
		<form onSubmit={handleVerifySubmit} className="flex w-[427px] p-[30px] flex-col items-start gap-[38px] self-stretch">
			<div className="flex flex-col items-start gap-3 self-stretch">
				...
				<h2 className="text-[var(--text-secondary)]">
					เราได้ส่งรหัส 6 หลักไปที่อีเมล {email}
				</h2>
			</div>
			...
			<div className="flex w-full flex-col items-start gap-5 self-stretch">
				<label className="flex flex-col items-start gap-2 self-stretch">
					<div className="flex justify-between items-center self-stretch">
						...
						<button type="button" onClick={handleResendCode} disabled={!canResend || loading} className="flex justify-center items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed">
							...
							<span className="text-[var(--primary)]">
								{canResend ? "ส่งรหัสใหม่" : `ส่งรหัส (${formatTime(timeLeft)})`}
							</span>
						</button>
					</div>
					<div className="flex h-[44px] py-0 px-3.5 items-center gap-2.5 self-stretch rounded-lg border-[1px] border-solid border-[var(--border)]">
						...
						<div className="w-full text-[var(--text-secondary)]">อีเมล ({email})</div>
			...
			<div className="flex w-full justify-around gap-2.5 self-stretch">
				{otp.map((digit, index) => (
					<input key={index} ref={(el) => {(inputRefs.current[index] = el)}} className="flex w-full h-[44px] p-[10px] items-start gap-2.5 flex-[1_0_0] rounded-lg border-[1px] border-solid border-[var(--border)] bg-[var(--input-default)] outline-none text-center text-lg font-semibold focus:border-[var(--primary)] transition-colors" type="text" inputMode="numeric" autoComplete="off" maxLength={1} value={digit} onChange={(e) => handleChange(index, e.target.value)} onKeyDown={(e) => handleKeyDown(index, e)} onPaste={index === 0 ? handlePaste : undefined} disabled={loading} />
				))}
			</div>

			<div className="flex w-full flex-col gap-5 self-stretch">
				<button className="flex w-full h-[44px] justify-center items-center gap-2.5 self-stretch rounded-lg bg-[var(--primary)] text-[var(--primary-foreground)] disabled:opacity-50 disabled:cursor-not-allowed font-medium hover:opacity-90 transition-opacity" type="submit" disabled={loading || !isFormValid}>
					{loading ? "กำลังยืนยัน..." : "ยืนยัน"}
				</button>
				<div className="flex justify-center self-stretch">
					<button type="button" className="flex gap-2.5 cursor-pointer hover:opacity-80 transition-opacity" onClick={() => setShowMfa(false)} disabled={loading}>
						...
						<div className="text-[var(--text-secondary)] font-medium leading-normal not-italic">
